...
export function CreditProfilePanel() {
  const updateCreditProfile = useObjectiveStore((state) => state.updateCreditProfile);
  const getBorrowerRanking = useObjectiveStore((state) => state.getBorrowerRanking);
  ...
  return (
    <Stack gap="lg">
      <Paper p="md" withBorder>
        <Group justify="space-between" mb="md">
          ...
          <Group>
            <Badge size="lg" color="green">
              优秀信用: {ranking.filter((p) => p.creditLevel === 'excellent').length}
            </Badge>
            <Badge size="lg" color="red">
              信用预警: {ranking.filter((p) => p.creditLevel === 'poor').length}
            </Badge>
          </Group>
        </Group>

        <Title order={5} mb="sm">信用排名 Top 5</Title>
        <SimpleGrid cols={{ base: 1, sm: 2, md: 5 }} mb="md">
          {ranking.slice(0, 5).map((profile, index) => (
            <Card key={profile.id} padding="md" withBorder>
              <Stack align="center" gap="xs">
                <Group gap="xs">
                  <IconTrophy size={20} color={index === 0 ? '#fbbf24' : index === 1 ? '#9ca3af' : '#d97706'} />
                  <Text fw={700}>#{index + 1}</Text>
                </Group>
                <Avatar size="lg" radius="xl">
                  <IconUser size={30} />
                </Avatar>
                <Text fw={500} ta="center">{profile.borrowerName}</Text>
                <RingProgress
                  size={80}
                  thickness={8}
                  roundCaps
                  sections={[{ value: profile.creditScore, color: getCreditColor(profile.creditScore) }]}
                  label={
        ...
        <Title order={5} mb="sm">信用详情</Title>
        <ScrollArea>
          <Table>
            <Table.Thead>
              <Table.Tr>
                ...
                <Table.Th>信用评分</Table.Th>
                <Table.Th>信用等级</Table.Th>
                <Table.Th>总借用次数</Table.Th>
                <Table.Th>按时归还</Table.Th>
                <Table.Th>超期次数</Table.Th>
                <Table.Th>累计超期天数</Table.Th>
                <Table.Th>损坏次数</Table.Th>
                <Table.Th>处罚次数</Table.Th>
              ...
              {ranking.map((profile, index) => (
                <Table.Tr key={profile.id}>
                  <Table.Td>
                    <Group gap="xs">
                      {index < 3 && (
                        <IconTrophy size={16} color={index === 0 ? '#fbbf24' : index === 1 ? '#9ca3af' : '#d97706'} />
                      )}
                  ...
                  <Table.Td>
                    <Group>
                      ...
                      <Text size="sm" fw={500}>{profile.borrowerName}</Text>
                    </Group>
                  </Table.Td>
                  <Table.Td>
                    <Group gap="sm" wrap="nowrap">
                      <Progress
                        ...
                        style={{ width: 100 }}
                      />
                      <Text size="sm" fw={500}>{profile.creditScore}</Text>
                    </Group>
                  </Table.Td>
                  <Table.Td>
                    <Badge color={CREDIT_LEVEL_COLORS[profile.creditLevel]}>
                  ...
                  <Table.Td>
                    <Text size="sm">{profile.totalBorrows}</Text>
                  </Table.Td>
                  <Table.Td>
                    <Group gap="xs">
                  ...
                  <Table.Td>
                    {profile.overdueCount > 0 ? (
                      <Group gap="xs">
                        ...
                        <Text size="sm" c="red">{profile.overdueCount}</Text>
                      </Group>
                    ) : (
                      <Text size="sm">0</Text>
                    )}
                  ...
                  <Table.Td>
                    <Text size="sm">{profile.damageCount}</Text>
                  </Table.Td>
                  <Table.Td>
                    <Text size="sm">{profile.penaltyCount}</Text>
                  </Table.Td>
                  <Table.Td>
                    <Button
                      size="xs"
      ...
      <Paper p="md" withBorder>
        <Title order={5} mb="sm">信用评级说明</Title>
